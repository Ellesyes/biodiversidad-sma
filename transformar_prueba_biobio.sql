-- ============================================================
-- PRUEBA: misma transformación que transformar.sql, pero
-- filtrada solo a la región Biobío, para validar rápido.
-- ============================================================
...
SELECT DISTINCT ON (datos->>'CampanaSmaId')
    datos->>'CampanaSmaId', datos->>'DocumentoId', datos->>'NombreCampana',
    safe_int(datos->>'AnoInicio'),
    safe_date(datos->>'FechaInicio'),
    safe_date(datos->>'FechaTermino'),
    datos->>'ObjetivoCampana', datos->>'InformeId'
FROM raw_monitoreo
WHERE datos->>'CampanaSmaId' IS NOT NULL AND region_archivo = 'Biobio'
...
SELECT DISTINCT ON (datos->>'EstacionReplicaSmaId')
    datos->>'EstacionReplicaSmaId', datos->>'NombreEstacion', datos->>'TipoMonitoreo',
    datos->>'RescateAdaptacionRelocalizacion', datos->>'Region', datos->>'Provincia',
    datos->>'Comuna', datos->>'Localidad', datos->>'EcosistemaNivel1', datos->>'EcosistemaNivel2', datos->>'Exposicion',
    safe_numeric(datos->>'LargoM'), safe_numeric(datos->>'AnchoM'),
    safe_numeric(datos->>'RadioM'), safe_numeric(datos->>'SuperficieM2'),
    safe_numeric(datos->>'LatitudDecimalCentral'), safe_numeric(datos->>'LongitudDecimalCentral')
FROM raw_monitoreo
WHERE datos->>'EstacionReplicaSmaId' IS NOT NULL AND region_archivo = 'Biobio'
...
WHERE r.datos->>'OcurrenciaSmaId' IS NOT NULL AND r.region_archivo = 'Biobio'
...
LEFT JOIN dim_taxon t
    ...
    AND t.epiteto_especifico IS NOT DISTINCT FROM r.datos->>'EpitetoEspecifico'
    AND t.epiteto_infraespecifico IS NOT DISTINCT FROM r.datos->>'EpitetoInfraespecifico'
WHERE r.datos->>'CapturaSmaId' IS NOT NULL AND r.region_archivo = 'Biobio'
...
LEFT JOIN dim_taxon t
    ...
    AND t.epiteto_especifico IS NOT DISTINCT FROM r.datos->>'EpitetoEspecifico'
    AND t.epiteto_infraespecifico IS NOT DISTINCT FROM r.datos->>'EpitetoInfraespecifico'
WHERE r.datos->>'AdaptacionSmaId' IS NOT NULL AND r.region_archivo = 'Biobio'
